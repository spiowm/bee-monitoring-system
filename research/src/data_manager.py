...
def download_dataset(kaggle_id: str) -> Path:
    """Download dataset via kagglehub (uses cache) and return path to data root."""
    import kagglehub
    _kaggle_token()
...
def ensure_dataset(config_dict: dict) -> None:
    ...
    if source_type == "kaggle":
        ...
        raw_dir = download_dataset(kaggle_id)
    else:
        raw_dir = Path(data.get("raw_dir", ""))
        if not raw_dir or not raw_dir.exists():
            raise FileNotFoundError(f"raw_dir не знайдено: {raw_dir}")

    prepare_dataset(raw_dir, data.get("prepared_dir", "datasets/prepared"), data)
...
if __name__ == "__main__":
    ...
    refs = sys.argv[1:] or [
        "spiowm/bee-monitoring-pose",
        "spiowm/bee-monitoring-ramp-detection",
    ]
    for ref in refs:
