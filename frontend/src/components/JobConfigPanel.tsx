...
export default function JobConfigPanel({
  ...
  const { data: availableModels = [] } = useQuery<{ name: string }[]>({
    queryKey: ['models'],
    queryFn: async () => {
  ...
  return (
    <div className="card space-y-4">
      ...
      {/* Line position */}
      <Slider label="Line Position" value={config.line_position ?? 0.5} min={0.1} max={0.9} step={0.05} onChange={v => set({ line_position: v })} />
      ...
      {showAdvanced && (
        <div className="space-y-3 bg-[var(--bg-panel)] p-3 rounded-lg border border-gray-800">
          <Slider label="Detection Confidence" value={config.conf_threshold ?? 0.35} min={0.1} max={0.9} step={0.05} onChange={v => set({ conf_threshold: v })} />
